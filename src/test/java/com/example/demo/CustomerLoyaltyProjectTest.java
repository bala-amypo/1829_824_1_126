...
/**
 * CUSTOMER LOYALTY PROJECT – TEST CASES (1–70)
 */
...
public class CustomerLoyaltyProjectTest {
    ...
    public void testCreateCustomer() {
        Assert.assertNotNull(activeCustomer.getId());
    }
    ...
    public void testGetPurchasesByCustomer() {
        Assert.assertNotNull(purchaseService.getPurchasesByCustomer(activeCustomer.getId()));
    }

    @Test(priority = 17)
    public void testGetVisitsByCustomer() {
        Assert.assertNotNull(visitService.getVisitsByCustomer(activeCustomer.getId()));
    }

    @Test(priority = 18)
    public void testInvalidCustomerLookup() {
        Assert.assertFalse(customerService.findByCustomerId("XXX").isPresent());
    }

    @Test(priority = 19)
    public void testInvalidRuleLookup() {
        Assert.assertFalse(ruleService.getRule("GOLD", "BRONZE").isPresent());
    }
    ...
    // (Fake services exactly as required for test execution)
